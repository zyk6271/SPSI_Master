...
#else
    LOG_I("system watch test thread startup, name = %.*s, priority = %d", RT_NAME_MAX, thread->parent.name, thread->current_priority);
    LOG_I("system watch thread exception timeout = %d s, delay = %d s", SYSWATCH_EXCEPT_TIMEOUT, delay_s);
    LOG_I("system watch exception resolve mode = %d", SYSWATCH_EXCEPT_RESOLVE_MODE);

    rt_thread_mdelay(delay_s * 1000);
    LOG_I("%.*s thread exception begin. priority = %d", RT_NAME_MAX, thread->parent.name ,thread->current_priority);
    while(1);
#endif
}
